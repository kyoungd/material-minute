...
class FilterDailySupplyDemandZone:
    def __init__(self, marketOpenAt = None):
        ...
        self.minMaxRangePercent = 0.05

    def getMaxMin(self, df: pd.DataFrame):
    ...
    def IsNearMaxMin(self, df: pd.DataFrame, close:float) -> bool:
        high, low = self.getMaxMin(df)
        priceDelta = close * self.minMaxRangePercent
        if abs(high - close) <= priceDelta:
            return True
        if abs(low - close) <= priceDelta:
            return True
        return False
    ...
    def Run(self, symbol: str, df: pd.DataFrame, close: float) -> bool:
        try:
            if len(df) <= 3:
                return False
            return self.IsNearMaxMin(df, close)
