...
class FilterDailySupplyDemandZone:
    ...
    def getMinmaxOnClose(self, df: pd.DataFrame) -> {}:
        high = None
        low = None
    ...
    def firstKeyPoints(self, dataf: pd.DataFrame) -> []:
    ...
    def Run(self, symbol: str, df: pd.DataFrame, close: float) -> bool:
        try:
            ...
            firstkeys = self.firstKeyPoints(keypoints[::-1])
            for key in firstkeys[0:-1]:
                if FilterPriceSpread.IsNearPrice(key, close, self.minMaxNearPercent):
                    return True
            return False
...
class FilterDailySupplyDemandSimple:
